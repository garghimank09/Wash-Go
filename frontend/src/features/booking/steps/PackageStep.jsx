...
export function PackageStep({
  ...
  const selectedPkg = tiers.find((p) => p.id === packageId);
      ...
      <div>
        ...
        {tiersLoading ? (
          <div className="mt-3 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
            {[0, 1, 2, 3].map((k) => (
              <Card key={k} className="min-h-[180px] p-4">
                <Skeleton height={40} width={40} borderRadius={12} className="mb-3" />
                <Skeleton height={18} width="60%" />
                <Skeleton height={12} width="80%" className="mt-2" />
                <Skeleton count={3} height={10} className="mt-4" />
              </Card>
            ))}
          </div>
        ) : tiers.length === 0 ? (
          <p className="mt-3 text-sm text-wg-muted">No wash tiers available. Contact support or try again later.</p>
        ) : (
          <m.div
          ...
          >
            {tiers.map((p) => {
              const Icon = PKG_ICONS[p.icon] ?? Sparkles;
        ...
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-1 max-w-md text-xs leading-relaxed text-wg-muted">
              {selectedPkg?.label ?? 'Selected tier'} · estimate for your size — final price confirmed at booking.
            </p>
          </div>
        </div>
