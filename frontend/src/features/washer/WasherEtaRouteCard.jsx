...
export function WasherEtaRouteCard({ etaMinutes, address, phase, tracking }) {
  ...
  const useLiveMap = Boolean(tracking?.customer);
  const eta = tracking?.eta_minutes ?? etaMinutes;
  const mapLive = Boolean(tracking?.live);
  const mapSim = Boolean(tracking?.simulated);
      ...
      <div className={cn('relative w-full', useLiveMap ? 'h-52 sm:h-56' : 'h-[10rem] sm:h-44')}>
        ...
        <div className="absolute inset-x-0 top-3 z-10 flex items-start justify-between px-4 pointer-events-none">
          ...
          {useLiveMap && mapLive ? (
            ...
            <span className="rounded-full border border-cyan-400/35 bg-cyan-500/20 px-2 py-1 text-[10px] font-bold uppercase tracking-wide text-cyan-50">
              Live route
            </span>
          ) : liveNav ? (
            <span className="flex items-center gap-1.5 rounded-full border border-emerald-400/35 bg-emerald-500/20 px-2 py-1 text-[10px] font-black uppercase tracking-wide text-emerald-50 shadow-sm">
          ...
          <div>
            ...
            {atSite ? (
              ...
              <p className="mt-1 text-xs font-medium text-cyan-100/90">
                {mapLive
                  ? 'Navigate to customer · live GPS'
                  : useLiveMap
                    ? `En route · ${tracking?.distance_km != null ? `${tracking.distance_km} km` : 'local route'}`
                    : 'Moving · traffic-aware (demo)'}
              </p>
            ) : (
