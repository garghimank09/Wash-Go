...
export function AdminActiveBookingsMonitor({ rows }) {
      ...
      <div className="border-b border-white/10 px-4 py-3 dark:border-white/5">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <div>
            ...
            <p className="mt-0.5 text-xs text-wg-muted">Field phases & ETA slip (mock telemetry).</p>
          </div>
      ...
      <div className="max-h-[220px] overflow-y-auto">
        <table className="w-full min-w-[560px] text-left text-xs">
          ...
          <tbody className="divide-y divide-wg-border/60">
            {rows.map((r) => (
              <tr key={r.id} className="bg-white/[0.02] dark:bg-transparent">
